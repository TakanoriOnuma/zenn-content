...
テストの動作確認は、Storybookでは`Dnd/Default Play`と`Dnd/Overlay Play`がplay functionを設定したものを確認できます。Vitestのhappy-domやjsdomを使ったテストはそれぞれ`npm run test:happy-dom`、`npm run test:jsdom`を実行することで確認できます。
